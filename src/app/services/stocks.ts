...
export class StocksService {
  ...
  GetStocks(): Observable<Stock[]> {
    const required: string[] = this.stocks
      .filter(d => !this.stockPrices[d.Symbol])
      .map(d => d.Symbol);
    return this._quoteService.getPrices(required).map(prices => {
  ...
  AddStock(stock: QuoteSearch, quantity: number): void {
    this.stocks.push(
      <Stock>{ Symbol: stock.symbol, Name: stock.name, Quantity: quantity }
    );
    this.SaveToLocalStorage();
  }
